...
#define DEFAULT_GPIO_TRIGGER 12
...
void pwm_setRange(int range) {
	(*(pwm_Vaddr + PWM_RNG1)) = range;
	udelay(15);
}

void pwm_setData(int data) {
	(*(pwm_Vaddr + PWM_DAT1)) = data;
	udelay(15);
}
...
static int servo_write(struct file *pfile, const char __user *buffer, size_t length, loff_t *offset){
	int buffer_size;
	buffer_size = length;
...
static int servo_open(struct inode *pinode, struct file *pfile) {
	

	printk(KERN_ALERT "GPIO: %d <<< SERVO\n", gpio_trigger);
...
static int servo_release(struct inode *pinode, struct file *pfile) {
...
static struct miscdevice servo_driver = {
...
static int __init servo_init(void){
	...
	err = misc_register(&servo_driver);
	printk(KERN_ALERT "SERVO_INIT \n");
